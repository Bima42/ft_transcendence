...
import { PongConfig } from "../GameConfig"
import { WorldStateDto } from "../WorldStateDto"
...
export default class PongScene extends Phaser.Scene {
  ...
  private parseConfig(config: PongConfig) {
    this.config.maxScore = config.maxScore ?? 3;
    this.config.customPong = config.customPong ?? true;
  }

    updateWorldFromServer(obj: WorldStateDto) {
      this.paddle1.y = obj.p1Y;
      this.paddle2.y = obj.p2Y;
      this.ball.x = obj.ball.x;
      this.ball.y = obj.ball.y;
      this.ball.setVelocity(obj.ball.vx, obj.ball.vy);
    }

    create(config: PongConfig) : void
    {
      this.parseConfig(config);
      if (this.config.customPong) {
        ...
        console.log("Classic game.");
          ...
          this.socket.on("state", (msg: string) => {
             try {
                 const worldState: WorldStateDto = JSON.parse(msg);
                 this.updateWorldFromServer(worldState);
             } catch (error) {
      ...
      if (this.config.customPong)
      {
        this.obstacles.push(new Obstacle(this, 200, 300, 'red'));
        this.obstacles.push(new Obstacle(this, 300, 200, 'yellow'));
        this.obstacles.push(new Obstacle(this, 600, 500,'blue'));
        this.obstacles.push(new Obstacle(this, 500, 100, 'green'));
  ...
  hitObstacle(ball : Ball, obstacle: Obstacle) {
    ...
    hitPaddle(ball: Ball, paddle: Paddle)
    {
        const minAngle = 30;
